...
class RobotContainer:
    ...
    def __init__(self, robot) -> None:
        ...
        self.frontLimelight = LimelightCamera("limelight-front")
        self.backLimelight = LimelightCamera("limelight-back")
    ...
    def configureFeederApproachButtons(self):
        ...
        def rightApproachHeadingDegrees():
            return 54 if DriverStation.getAlliance() == DriverStation.Alliance.kBlue else 180 + 54

        def leftApproachHeadingDegrees():
            return -54 if DriverStation.getAlliance() == DriverStation.Alliance.kBlue else 180 - 54
        ...
        self.driverController.axisGreaterThan(XboxController.Axis.kRightTrigger, 0.1).whileTrue(
            approachFeeder(
                headingDegrees=rightApproachHeadingDegrees,  # right feeder
                speed=rightApproachSpeed
            )
        )
        self.driverController.axisGreaterThan(XboxController.Axis.kLeftTrigger, 0.1).whileTrue(
            approachFeeder(
                headingDegrees=leftApproachHeadingDegrees,  # left feeder
    ...
    def getBlue1Station(self):
        ...
        return startToReef().andThen(reefToStation()).andThen(stationToScore()).andThen(scoreToStation()).andThen(
            repeatStationToScore())
    ...
    def getBlue3Station(self):
        def startToReef():
            startLocation = ResetXY(x=7.600, y=0.795, headingDegrees=180, drivetrain=self.robotDrive)
            setPipeline3 = SetCameraPipeline(camera=self.frontLimelight, pipelineIndex=3)
            aim120 = GoToPoint(x=6.198, y=1.789, speed=0.8, finishDirection=Rotation2d.fromDegrees(120), drivetrain=self.robotDrive
                               ).until(self.frontLimelight.hasDetection)
            approachTag = ApproachTag(camera=self.frontLimelight, drivetrain=self.robotDrive, speed=1.0,
                                      pushForwardSeconds=None,
                                      specificHeadingDegrees=120)
            level2 = SetElevatorPosition(elevator=self.elevator, position=4.5)
            ...
            return startLocation.andThen(setPipeline3).andThen(aim120).andThen(getToReef).andThen(score)

        def reefToStation():
            level0 = SetElevatorPosition(elevator=self.elevator, position=0.1)
            swerveRight = SwerveMove(metersToTheLeft=1.0, metersBackwards=0.5, drivetrain=self.robotDrive,
                                       speed=0.5, heading=Rotation2d.fromDegrees(20))
            setPipeline1 = SetCameraPipeline(camera=self.backLimelight, pipelineIndex=1)
            parallelCommand = swerveRight.andThen(setPipeline1).alongWith(level0)
            approachFeeder = ApproachTag(camera=self.backLimelight, drivetrain=self.robotDrive, speed=1.0,
                                         pushForwardSeconds=1.0, specificHeadingDegrees=54, reverse=True,
                                         finalApproachObjSize=0.8,
                                         settings={
                                             "GainTran": 0.7,
                                             "Tolernce": 14,
                                         })
            intake = IntakeGamepiece(self.intake, speed=-0.2).withTimeout(0.5)

            return parallelCommand.andThen(approachFeeder).andThen(intake)

        def stationToScore():
            setPipeline3 = SetCameraPipeline(camera=self.frontLimelight, pipelineIndex=3)
            finishIntaking = IntakeGamepiece(self.intake, speed=-0.2)
            approachScore = ApproachTag(camera=self.frontLimelight, drivetrain=self.robotDrive, speed=1.0,
                                        pushForwardSeconds=None, specificHeadingDegrees=60)
            scoreLevel1 = IntakeFeedGamepieceForward(self.intake, motor1speed=-0.5, motor2speed=-0.2).withTimeout(1.5)

            return setPipeline3.andThen(approachScore.alongWith(finishIntaking)).andThen(scoreLevel1)

        def scoreToStation():
            setPipeline1 = SetCameraPipeline(camera=self.backLimelight, pipelineIndex=1)
            approachStation = ApproachTag(camera=self.backLimelight, drivetrain=self.robotDrive, speed=1.0,
                                          pushForwardSeconds=None, specificHeadingDegrees=54)
            intake = IntakeGamepiece(self.intake, speed=-0.2)
    ...
    def getRed1Station(self):
        def startToReef():
            ...
            return startLocation.andThen(setPipeline3).andThen(aim40).andThen(getToReef).andThen(score)
        ...
        def reefToStation():
            ...
            parallelCommand = ParallelDeadlineGroup(swerveRight, setPipeline1)
            approachFeeder = ApproachTag(camera=self.backLimelight, drivetrain=self.robotDrive, speed=1.0,
                                         pushForwardSeconds=None, specificHeadingDegrees=126)
            intake = IntakeFeedGamepieceForward(self.intake, motor1speed=-0.2)
        ...
        def stationToScore():
            setPipeline3 = SetCameraPipeline(camera=self.frontLimelight, pipelineIndex=3)
            approachScore = ApproachTag(camera=self.frontLimelight, drivetrain=self.robotDrive, speed=1.0,
                                        pushForwardSeconds=None, specificHeadingDegrees=120)
            scoreLevel1 = IntakeFeedGamepieceForward(self.intake, motor1speed=-0.5, motor2speed=-0.2)
